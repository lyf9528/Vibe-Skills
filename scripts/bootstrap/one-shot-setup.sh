...
prompt_for_host_id() {
  ...
  echo "  3) windsurf     - runtime-core preview lane"
  while true; do
    ...
    case "${normalized}" in
      ...
      3|windsurf) HOST_ID='windsurf'; return 0 ;;
      *)
...
ensure_requested_host_id() {
  ...
  echo "[FAIL] Pass --host codex|claude-code|windsurf when running non-interactively." >&2
  return 1
}
