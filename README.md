...
| 多代理并发的时候，处理多种不同任务时，不好指定分发对应技能                                                                                                                        | 对于多代理会自动分发相关任务的对应skills  |
...
**安装与配置指南**
* 当前公开支持面：**`codex`、`claude-code`、`cursor`、`windsurf`**
  其中 `codex` 是当前最完整的 governed 路径，`claude-code` 与 `cursor` 提供支持的安装与使用路径，`windsurf` 提供支持的安装与使用路径且已接入 runtime adapter。
